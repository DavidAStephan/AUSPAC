...
var
    ...
    pv2_yh          // PV of PV — nested expectation of permanent income (FR-BDF Table 4.6.4)
    ...
    pv_rKB          // PV of expected future user cost changes (FR-BDF eq 64: σ·PV(Δlog r̂_KB))
    ...
    pv_i_uip        // forward PV of policy-rate gap for UIP (MCE: fully forward)
...
varexo
    ...
    eps_e_pQ_star   // VA price target shock
    eps_e_c_star    // consumption target shock
    eps_e_ib_star   // business investment target shock
    eps_e_ih_star   // household investment target shock
    eps_e_n_star    // employment target shock
;
...
parameters
    ...
    b_di_c          // interest rate CHANGE sensitivity (FR-BDF eq 61 beta_3)
    ...
    b_ph_ih         // housing price gap in short-run PAC (FR-BDF eq 67 beta_3)
    ...
    beta_uip        // UIP forward-NPV discount (MCE: fully forward-looking)
...
rho_pcom        = 0.42;     // commodity price persistence
...
b_ph_ih         =  0.0099;  // Phase C spliced 1959Q3+ housing-price series; IV (F=432.1, T=115); sign now correct, magnitude near zero
...
beta_uip        = 0.92;     // UIP forward-NPV discount (MCE: jump on impact -> AUD appreciation jumps)
...
b1_x            = 0.30;     // Phase D v2 (ABS 5206 SA, T=126 + COVID dummies): OLS=-0.194 wrong sign; the 0.807 from Trend was smoothing artifact. Kept FR-BDF 0.30.
b2_x            = 0.25;     // Phase D v2 (ABS 5206 SA T=126): OLS=-0.008, t=-0.07. US output gap is the wrong demand proxy — Asian PMI needed (Phase K).
...
b1_m            = 0.2316;   // Phase D v3 (ABS 5206 SA + IAD + COVID dummies): OLS=+0.232 (s.e. 0.086, t=2.71)
b2_m            = 0.3591;   // Phase D v3 (ABS 5206 SA + IAD demand index): OLS=+0.359 (s.e. 0.101, t=3.56) — Phase K residual RESOLVED
...
rho_pg          = 0.13;     // government deflator moderately persistent
alpha_pg        = 0.37;     // VA price pass-through to government prices
...
beta_pm_com     = 0.42;     // commodity price -> import deflator
...
pac_model(discount = beta_pac, model_name = pac_ih, growth = ih_star_l(-1));
pac_model(discount = beta_pac, model_name = pac_n, growth = n_star_l(-1));
...
model;
    ...
    // Nested PV (PV²): expectation of the expectation of permanent income.
    // FR-BDF Table 4.6.4: the consumption target depends on PV of PV(yH),
    // not just PV(yH) directly. This creates smoother, more persistent income expectations.
    // At SS: pv_yh = 0 => pv2_yh = 0.
    [name = 'eq_pv2_yh']
    pv2_yh = (1 - beta_c) * pv_yh + beta_c * pv2_yh(+1);

    // Consumption target (paper eq 59): c* = a0 + PV²(yH) + alpha1*(rLH - r_bar).
    // Uses pv2_yh (nested PV) instead of pv_yh for the permanent income channel.
    // Real lending rate gap: (i_lh - pi_c) - (i_ss + tp_ss + spread_lh - pi_ss_au).
    // At SS: pv2_yh=0, i_lh=SS, pi_c=pi_ss => dln_c_star_bar = 0.
    [name = 'eq_dln_c_star_bar']
    dln_c_star_bar = kappa_inc * (pv2_yh - pv2_yh(-1))
    ...
    diff(ln_c_level) = b0_c * (c_star_l(-1) - ln_c_level(-1))
            ...
            + b2_c * i_gap(-1)
            // b_di_c * di_gap: disabled (b_di_c=0; inline diff crashes Dynare 6.5 preprocessor)
            + b3_c * yhat_au
            + eps_c;
    ...
    // PV of expected future user cost changes (FR-BDF eq 64: σ·PV(Δlog r̂_KB))
    // Forward-looking: agents discount future user cost changes by beta_pac.
    // At SS: dln_uc_k = 0 => pv_rKB = 0.
    [name = 'eq_pv_rKB']
    pv_rKB = (1 - beta_pac) * dln_uc_k + beta_pac * pv_rKB(+1);
    ...
    // User cost: sigma_ces * pv_rKB (FR-BDF eq 64: σ·PV(Δlog r̂_KB), structural forward)
    ...
    // Rate channel via pac_expectation (b4_ih dropped: F=0.001).
    ...
    // Real exchange rate gap follows forward-looking modified UIP (MCE).
    // Phase Q (2026-05-15): pv_i_uip is fully forward-looking, so the entire
    // expected rate path is internalised in the UIP equation on impact.
    //
...
steady_state_model;
    ...
    pv_rKB         = 0;            // PV of user cost changes = 0 at SS
