...
var
    ...
    pv_i_uip        // backward PV of policy-rate gap for UIP (VAR regime AR(1))
...
parameters
    ...
    beta_uip        // UIP NPV discount (VAR regime: backward AR(1) form)
...
beta_uip        = 0.92;     // UIP NPV discount (VAR regime: backward AR(1) — slow build-up, no impact jump)
...
// Dynamic E-SAT auxiliary parameters — AU Bayesian posteriors (Phase B, 2026-05-09)
// See dynare/auxiliary_bayesian_results.txt for 90% CIs and OLS comparators.
rho_pQ_aux = 0.334; a_pQ_y = 0.043; a_pQ_i = -0.021; a_pQ_pi = 0.007;  a_pQ_u = -0.021;
rho_n_aux  = 0.743; a_n_y  = 0.094; a_n_i  = -0.031; a_n_pi  = 0.057;  a_n_u  = -0.029;
rho_yh_aux = 0.93;  a_yh_y = 0.12;  a_yh_u = -0.07;  // unchanged: YH/Y data unavailable
rho_c_aux  = 0.581; a_c_y  = 0.058; a_c_i  = -0.043; a_c_pi  = 0.010;  a_c_u  = -0.036; a_c_yh = 0.10;
rho_ib_aux = 0.694; a_ib_y = 0.050; a_ib_pi = 0.023; a_ib_u  = 0.004;
rho_rKB_aux = 0.162; a_rKB_i = 0.242;
rho_ih_aux = 0.699; a_ih_y = 0.097; a_ih_i = -0.152; a_ih_pi = 0.042;  a_ih_u  = 0.004;
...
model;
    ...
    // Backward form for VAR-based variant (no leads allowed under VAR expectations).
    // FR-BDF eq 61 includes α_1·PV(r_LH) channel.
    // At SS: i_lh=SS, pi_c=pi_ss => bracket=0 => pv_r_lh_gap=0.
    [name = 'eq_pv_r_lh_gap']
    pv_r_lh_gap = beta_c * pv_r_lh_gap(-1)
                + (1 - beta_c) * (i_lh - pi_c - (i_ss + tp_ss + spread_lh - pi_ss_au));
    ...
    // Rate channel via pac_expectation + pv_ih_aux (b4_ih dropped: F=0.001).
    ...
    // Real exchange rate gap follows backward-looking modified UIP under VAR.
    // Phase Q (2026-05-15): pv_i_uip is now backward AR(1), the E-SAT-style
    // policy function for the PV of the rate gap. Under VAR expectations
    // agents do not anticipate the persistent rate path on impact, so
    // pv_i_uip(0) = 0 and the AUD response builds up slowly — matching the
    // FR-BDF VAR baseline (no Hybrid amplification through the trade channel).
    //
...
steady_state_model;
    ...
    pv_i_uip       = 0;                             // backward UIP PV = 0 at SS
