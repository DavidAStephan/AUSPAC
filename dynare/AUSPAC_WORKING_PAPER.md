...
> - **Two corrections in progress (Wave 1).** (i) After the §6.13 SA-data fix, the `h_pac_n_*`/`h_pac_ih_*` policy-function vectors had not been regenerated, so the forward-looking PAC expectation was momentarily inconsistent with the new ECM speeds; this is being re-derived via `pac.print()`. (ii) The `σ_ces·rw_gap` term in `dln_n_star_bar` produces a permanent (and at long horizons wrong-signed) potential-output response to a temporary nominal shock; it is being corrected toward long-run neutrality. IRF numbers quoted above (real-GDP trough −0.073% etc.) predate these two fixes and will be refreshed.
