...
gives log marginal density Laplace = \textbf{−662.27}, Modified Harmonic
Mean = \textbf{−665.51} (20k×2-chain MCMC, posterior cached in
\texttt{dynare/au\_pac\_bayesian/}; cumulative Phase Q → L2 MHM
improvement of +136.8 nats). The wage Phillips curve has standard New
...
cash-rate tightening produces a real GDP trough of −0.186\% at Q7, an
output-gap trough of −0.123\% at Q7, CPI inflation falling −0.044 pp
year-on-year at Q15, the AUD appreciating 0.964\% at Q8 via a
forward-looking UIP NPV channel, and the 10-year yield rising 0.100 pp
on impact. A cross-block finding emerges from the Phase L2 estimates:
\textbf{AU ECM speeds are 4--8× faster than France's} in four of five
blocks (consumption is the exception); plausible drivers are AU's more
flexible price/wage setting (mining cycle, commodity-pass-through,
variable-rate mortgages). Benchmarking against the RBA's model suite
(Beckers 2020 VAR, MARTIN, DINGO, Murphy --- Mulqueeney, Ballantyne and
Hambur 2025) places AU-PAC at the conservative end of the AU range, with
magnitudes that reflect the greater capital--labour substitutability of
the CES specification (σ = 0.54) and the small estimated PAC
adjustment-cost coefficients. The forward-guidance puzzle is absent:
AU-PAC tracks the linear reference response to within 16\% even at N =
12 successive rate-cut announcements, while standard New Keynesian
models saturate near 1.8×. Australia-specific features include an
endogenous Taylor rule for the RBA, variable-rate mortgage transmission,
commodity-price channels, the US as the foreign bloc, and a trade ECM
calibrated to Australia's secular openness rise. The model is
implemented in Dynare 6.5.
...
the production model, which includes ULC/UCK structural channels,
deflator ECMs, endogenous spread widening, and energy/non-energy import
split. The magnitudes quoted in §7.2's channel-by-channel walkthrough
reflect this latest run.
...
\textbf{Log marginal data density}: Laplace = \textbf{−662.27}, MHM =
\textbf{−665.51} (+31.9 nats vs pre-fix; +136.8 nats cumulative from
Phase Q). Estimated on 20k×2-chain MH, MATLAB R2026a + Dynare 7.0
(native ARM, parallel pool with 6 workers), total wall time 15 min.
Three BI parameters (b0\_ib, b1\_ib, b3\_ib) are removed from
...
at the Bayesian posterior mean of Table 5.6 (Laplace = −662.27, MHM =
−665.51) under the CES calibration of Table 4.2.1 and the trade ECM of
§4.8. Peaks and troughs quoted in the text are read from the saved IRF
binaries; the corresponding figures are reproduced from the same
artefacts.
...
§7.2.2 channel-by-channel walkthrough reflect IRFs from this production
specification.
\end{quote}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Real GDP (ln\_Q)} & \textbf{−0.186\%} & Q7 \\
Output gap (yhat\_au) & \textbf{−0.123\%} & Q7 \\
CPI inflation (pi\_au, y/y) & \textbf{−0.044 pp} & Q15 \\
VA price inflation (piQ, y/y) & +0.037 pp & Q1 \\
CPI deflator inflation (pi\_c, y/y) & −0.054 pp & Q18 \\
Consumption growth (Δlog) & \textbf{−0.131\%} & Q2 \\
...
Exchange rate (s\_gap, \%) & \textbf{−0.964\%} & Q8 \\
10Y yield (annualised) & \textbf{+0.100 pp} & Q1 \\
\end{longtable}
}
...
The aggregate \textbf{−0.186\% real GDP} trough (Q7), \textbf{−0.123\%
output gap} trough (Q7), and \textbf{−0.044 pp y/y CPI inflation}
decline (Q15) decompose across six transmission channels documented in
Table 2.2.

\textbf{(i) Term structure and WACC.} The 10Y yield rises \textbf{+0.100
...
\textbf{−0.099\%} peak in \(\Delta \ln I^B\) at Q6. The structural form
(coef = +1 on PV(Δq̂) + PV(Δq̄), coef = −σ on PV(Δlog r̂\_KB) + PV(Δlog
...
Net peak in growth: \(\Delta \ln I^H = -0.034\%\) at Q6. The level
...
immediately. Result: s\_gap peaks at \textbf{−0.964\% at Q8}. Through
...
combined peak consumption-growth response is \textbf{−0.131\% at Q2} ---
...
with peak −0.038 pp y/y at Q19, driven by the CPI-passthrough channel
...
+0.037 pp y/y on impact (Q1)}, reflecting the ULC cost-push channel:
higher interest rates raise unit labour costs through the
user-cost-of-capital channel, and the ULC pass-through with
\(\gamma_{ulc} = 0.295\) transmits this into VA prices before the
demand-side deflationary forces dominate at longer horizons. The CPI
deflator inflation (pi\_c) peaks at −0.054 pp y/y at Q18.
...
\textbf{AU-PAC (Phase T, this paper)} & \textbf{−0.186\%} & \textbf{Q7}
& \textbf{−0.044} \\
\end{longtable}
}
...
The peak response timing is comparable to MARTIN/DINGO's short-horizon
range.
...
RBA suite: AUD appreciation peak −0.964\% at Q8. With ρ\_s = 0.775
...
\textbf{Verdict.} AU-PAC's output-gap trough (−0.123\% at Q7) is the
...
to −1.5\% in GDP terms). The real GDP trough (−0.186\% at Q7) is also at
the conservative end of the suite. The output-gap peak timing (Q7) sits
inside the MARTIN/DINGO short-horizon range. The smaller AU-PAC peak
magnitude reflects deliberate features of the PAC architecture:
adjustment-cost frictions economically motivated by Tinsley (1993) and
the FRB/US tradition, and Bayesian estimation on 30 years of AU data
that resists over-fitting. AU-PAC therefore sits within the
robust-policymaking spirit of ``diversity supports more robust
policymaking'' (Mulqueeney et al.~2025, p.~11) rather than aiming to
match the average response. Two refinements would narrow the gap:
external high-frequency RBA OIS-surprise data for \(b_{di,c}\)
identification, and adopting the broader FR-BDF 2026 financial-block
extensions (NFC accelerator, DSR-based mortgage block) to amplify the
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Monetary policy shock}]{paper_artifacts/irf_eps_i_v3.png}}
...
Consumption growth (−0.131\% peak at Q2) is the most immediately
...
Business-investment growth (−0.099\% at Q6) and housing-investment
growth (−0.034\% at Q6) follow with the lagged adjustment of the PAC
...
(s\_gap −0.964\% at Q8). Real GDP (\(\ln Q\)) troughs at
\textbf{−0.186\% at Q7}, and the output gap troughs at \textbf{−0.123\%
at Q7}.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Foreign demand shock}]{paper_artifacts/irf_eps_q_us_v3.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Government spending shock}]{paper_artifacts/irf_eps_g_v3.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Commodity price shock}]{paper_artifacts/irf_eps_pcom_v3.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Cost-push shock}]{paper_artifacts/irf_eps_pQ_v3.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={TFP shock}]{paper_artifacts/irf_eps_tfp_v3.png}}
\caption{TFP shock}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Term premium shock}]{paper_artifacts/irf_eps_tp_v3.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Output gap overview}]{paper_artifacts/irf_overview_output_v3.png}}
...
\item
  ...
  tightening produces a real-GDP trough of −0.186\% at Q7, an output-gap
  trough of −0.123\% at Q7, a CPI-inflation fall of −0.044 pp y/y at
  Q15, an AUD appreciation of 0.964\% at Q8 via the forward-NPV UIP, and
  a 10-year yield rise of 0.100 pp on impact via the term-structure NPV.
  The transmission combines cost-of-capital, exchange-rate,
...
and Hambur (2025). AU-PAC's output-gap trough (−0.123\% at Q7) and
real-GDP trough (−0.186\% at Q7) are at the low end of the RBA suite in
...
over-fitting. AU-PAC therefore complements the RBA suite on the
conservative end of ``diversity supports more robust policymaking''
