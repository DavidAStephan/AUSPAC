...
two-break trend-efficiency specification. AU-PAC contains 158 endogenous
variables, 40 exogenous shocks, and 270 parameters, with five PAC
behavioural equations governing value-added prices, consumption,
business investment, household investment, and employment, and a FR-BDF
error-correction trade block with long-run income and real-exchange-rate
elasticities. Expectations are formed as closed-form policy functions
obtained from inversion of the estimated core Expectation Satellite VAR
(E-SAT), supplemented by per-block auxiliary regressions following
wp1044 §3.2.3. The model is estimated on Australian quarterly data from
1994Q1 to 2024Q4. The authoritative calibration comes from full-system
Bayesian Metropolis-Hastings estimation of 28 joint parameters under an
11-observable specification; log marginal density Laplace = −781.05,
Modified Harmonic Mean = \textbf{−781.39}. The wage Phillips curve has
standard New Keynesian structure under genuine ABS Wage Price Index
data: λ\_w = 0.21 own-lag persistence, γ\_w = 0.35 CPI passthrough, and
a significant negative unemployment-PV coefficient κ\_w = −0.10
corresponding to a positive structural Phillips slope. The response to a
100 bp annualized cash-rate tightening produces a peak real GDP fall of
−0.22\% at Q7, an output-gap peak of −0.14\% at Q7, CPI inflation
falling 0.10 pp year-on-year, the AUD appreciating 0.97\% at Q8 via a
forward-looking UIP NPV channel, and the 10-year yield rising 0.41 pp on
impact. Benchmarking against the RBA's model suite (Beckers 2020 VAR,
MARTIN, DINGO, Murphy --- Mulqueeney, Ballantyne and Hambur 2025) places
AU-PAC at the conservative end of the AU range, with magnitudes that
reflect the greater capital--labour substitutability of the CES
specification (σ = 0.54) and the small estimated PAC adjustment-cost
coefficients. The forward-guidance puzzle is absent: AU-PAC tracks the
linear reference response to within 16\% even at N = 12 successive
rate-cut announcements, while standard New Keynesian models saturate
near 1.8×. Australia-specific features include an endogenous Taylor rule
for the RBA, variable-rate mortgage transmission, commodity-price
channels, the US as the foreign bloc, and a trade ECM calibrated to
Australia's secular openness rise. The model is implemented in Dynare
6.5.
...
\item
  ...
  Formation and the PAC Framework}
\item
  \hyperref[4-model-specification]{Model Specification}
\item
  \hyperref[5-estimation]{Estimation}
\item
  \hyperref[6-model-properties]{Model Properties}
\item
  \hyperref[7-conclusion]{Conclusion}
\item
  \hyperref[8-australia-specific-features]{Australia-Specific Features}
\end{enumerate}
...
Banque de France Working Paper \#736) to Australian data. The framework
belongs to the FRB/US family of semi-structural models used at central
banks worldwide, including MUSE at the Bank of Canada, LENS at the Bank
of England, and ECB-Base at the European Central Bank. These models
occupy a middle ground between reduced-form VARs (which lack structural
...
Forward-looking variables (hybrid) & 4 (pv\_i, pv\_i\_uip, pv\_u\_gap,
pv\_yh) \\
...
The model is implemented in Dynare 6.5. The production simulation model
\texttt{au\_pac\_v2.mod} is composed by Dynare's \texttt{aggregate()}
function from five per-PAC-block auxiliary files (one each for the
VA-price, consumption, business-investment, household-investment, and
employment PAC equations) plus a normalized identity layer; each
auxiliary file emits its closed-form policy-function PAC expectation
formula via \texttt{pac.print()} and the per-block equations are
stitched into the production model by \texttt{cherrypick()} (§4.4.0).
Legacy companion \texttt{.mod} files (\texttt{au\_pac.mod},
\texttt{au\_pac\_var.mod}, \texttt{au\_pac\_mce.mod}) for the
alternative expectation regimes of §2.2 are also provided. Source code
is available at \url{https://github.com/DavidAStephan/AUSPAC}. Bayesian
full-system estimation uses Dynare's \texttt{estimation()} command with
\texttt{diffuse\_filter} (required for the model's unit-root level
accumulators), a csminwel mode search followed by Metropolis-Hastings
MCMC with 20,000 draws across two chains.
...
framework and PAC microfoundations. Section 4 specifies all model
equations block by block, with coefficient tables for each. Section 5
describes the estimation strategy and results. Section 6 analyses model
properties including monetary policy transmission, impulse responses to
all shocks, conditional forecasting, and the forward guidance
experiment. Section 7 concludes. Section 8 discusses Australia-specific
features. Six appendices provide variable, shock, growth neutrality,
h-vector, parameter, and identification tables.
...
Backward (PAC h-vectors from var\_model) & 0 &
\texttt{au\_pac\_var.mod} \\
\textbf{Hybrid} & Forward (pv\_i, pv\_i\_uip, pv\_u\_gap, pv\_yh) &
Backward (PAC h-vectors from var\_model) & 4 & \texttt{au\_pac.mod} \\
\textbf{Full MCE} & Forward (all leads) & Forward (pac\_expectation
expands to leads) & 31 & \texttt{au\_pac\_mce.mod} \\
\end{longtable}
}

Under the baseline configuration, the five PAC expectations use
closed-form policy-function formulas in lagged structural state
variables (§4.4.0), while the term structure (\(pv_i\), eq. 48), UIP NPV
(\(pv_{i,uip}\), §4.10.3 --- forward NPV of the policy-rate gap),
unemployment PV, and permanent-income PV all use forward-looking
recursive forms. Under full model-consistent expectations (MCE), all
expectations are forward-looking. The forward-NPV UIP is what produces
the FR-BDF-style amplification of the exchange-rate channel documented
in §6.2.
...
\subsection{3. Expectation Formation and the PAC
Framework}\label{expectation-formation-and-the-pac-framework}
...
\bottomrule\noalign{}
\endlastfoot
piQ\_hat & 0.70 & 0.03 & -0.02 & 0.01 & -0.05 & --- & Table 4.4.4 \\
n\_hat & 0.67 & 0.12 & -0.03 & 0.05 & -0.04 & --- & Table 4.5.7 \\
yh\_ratio\_hat & 0.92 & 0.32 & --- & --- & -0.08 & --- & Table 4.6.3 \\
c\_hat & 0.60 & 0.06 & -0.04 & 0.01 & -0.03 & a\_c\_yh=0.39 & Table
4.6.4 \\
ib\_hat & 0.59 & 0.15 & --- & 0.04 & -0.02 & --- & Table 4.6.11 \\
rKB\_hat & 0.55 & --- & 0.24 & --- & --- & --- & Table 4.6.12 \\
ih\_hat & 0.71 & 0.08 & -0.08 & 0.05 & -0.03 & --- & Table 4.6.16 \\
\end{longtable}
}
...
command automates this computation.
...
\subsubsection{3.4 Three expectation
regimes}\label{three-expectation-regimes}

Under \textbf{VAR-based} expectations, \texttt{pac\_expectation()}
evaluates to \(k_0 Z_{t-1}\), a linear function of the lagged var\_model
state vector. All dynamics are backward-looking.

Under the \textbf{Hybrid} regime, PAC equations remain backward-looking
but three financial/labor variables use forward-looking recursive forms:
the term structure (\(pv_i\)), the present value of unemployment gaps
(\(pv_{u,gap}\)), and permanent income (\(pv_{yh}\)).

Under \textbf{full MCE}, \texttt{pac\_expectation()} expands into
forward-looking recursive leads of the target variable (wp736 eqs
138--142). The MCE version has 30 forward-looking variables compared to
3 under the hybrid regime.
...
The long run of the VA price is derived from the CES dual cost function
(eq. 13):

\[\pi^{Q*}_t = \rho^*_{pQ} \pi^{Q*}_{t-1} + \gamma_{ULC} \Delta \ln ULC_t + \gamma_{UCK} \Delta \ln UC^K_t + (1 - \rho^*_{pQ} - \gamma_{ULC}) \bar{\pi}_t\]

where \(\Delta \ln ULC = \pi_w - \Delta \ln Prod\) and
\(\Delta \ln UC^K = UC^K_t - UC^K_{t-1}\).

\subsubsection{Table 4.3.1: VA price target
coefficients}\label{table-4.3.1-va-price-target-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
Target persistence & rho\_pQ\_star & 0.95 & Calibrated \\
ULC pass-through & gamma\_ulc & \textbf{0.2951} &
\((1-\alpha)\sigma = 0.55 \times 0.5366\) (FR-BDF 2026 CES dual) \\
User cost pass-through & gamma\_uck & \textbf{0.2415} &
\(\alpha\sigma = 0.45 \times 0.5366\) (FR-BDF 2026 CES dual) \\
Inflation anchor & 1-rho-gamma & derived & Growth neutrality \\
\end{longtable}
}

\textbf{Growth neutrality}: At SS,
\(\pi^{Q*} = (\rho + \gamma_{ULC} + (1-\rho-\gamma_{ULC})) \times \pi_{SS} = \pi_{SS}\).
Verified.
...
\subsubsection{Table 4.3.2: VA price short-run
coefficients}\label{table-4.3.2-va-price-short-run-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
b0\_pQ (EC) & 0.060 & 0.027 & \textbf{0.0295} & {[}0.0067, 0.0513{]} \\
b1\_pQ (AR1) & 0.500 & 0.288 & \textbf{0.2728} & {[}0.1273, 0.4261{]} \\
b2\_pQ (output gap) & 0.090 & -0.014 & \textbf{0.0036} & {[}-0.0812,
0.0820{]} \\
...
The output gap coefficient (\(b^{pQ}_2\)) is statistically
indistinguishable from zero in the posterior (90\% HPD straddling 0),
indicating that the Phillips-curve channel operates primarily through
the target equation (ULC and user-cost pass-through with
\(\gamma_{ULC}+\gamma_{UCK}=0.54\) --- see §4.2) rather than the
short-run ad hoc term. Growth neutrality is verified:
\(1 - 0.293 - 0.452 \approx 0.26\).

\paragraph{4.3.3 E-SAT auxiliary (FR-BDF Table
4.4.4)}\label{e-sat-auxiliary-fr-bdf-table-4.4.4}

The VA price auxiliary equation in the var\_model (eq. 15):
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={VA Price Inflation: Dynamic Contributions}]{contrib_piQ.png}}
\caption{VA Price Inflation: Dynamic Contributions}
\end{figure}
...
visualisation\emph{: given the MCMC posterior parameters from Table 5.6,
Dynare's Kalman smoother reconstructs the unobserved auxiliary states
...
with \(\alpha_{pc} = 0.17\), \(\beta_{pc,m} = 0.10\),
\(\gamma_{oil} = 0.03\) (estimated single-equation OLS on AU data,
matching the \texttt{eq\_pi\_c} calibration). The reduced-form
persistence \(\lambda_\pi\) and Phillips slope \(\kappa_\pi\) are
jointly Bayesian-estimated with the rest of the model. This makes
\(\pi^{au}\) structurally a function of value-added price (\(\pi^Q\)),
import price (\(\pi^m\)), and commodity inflation (\(\Delta\ln pcom\)),
mirroring the role of \(\pi^C\) in the FR-BDF deflator block (wp736 eqs
79--80). Growth neutrality holds at the steady state:
\(\tilde{\pi}^{au}_t = 0\), \(\pi^Q = \bar{\pi}^{au}\),
\(\pi^m = \bar{\pi}^{au}\), \(\Delta\ln pcom = 0\) ⇒ all RHS terms
vanish.
...
The implementation uses Dynare 6.5's \texttt{var\_model} +
...
\textbf{Short-run PAC equation} (4th-order, FR-BDF eq 56, eq. 20):
...
\subsubsection{Table 4.4.4: Employment short-run PAC
coefficients}\label{table-4.4.4-employment-short-run-pac-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
b0\_n (EC) & 0.040 & 0.072 & \textbf{0.0616} & {[}0.0163, 0.1090{]} \\
b1\_n (AR1) & 0.300 & 0.323 & \textbf{0.3109} & {[}0.1459, 0.4808{]} \\
b2\_n (AR2) & 0.100 & -0.177 & --- & --- \\
b3\_n (AR3) & 0.050 & -0.081 & --- & --- \\
b4\_n (AR4) & 0.020 & -0.096 & --- & --- \\
b5\_n (output gap) & 0.120 & -0.017 & \textbf{0.0037} & {[}-0.0864,
0.0858{]} \\
stderr eps\_n & --- & --- & \textbf{0.3917} & {[}0.1221, 0.7059{]} \\
COVID crash & --- & -6.601 & --- & --- \\
COVID bounce & --- & +3.862 & --- & --- \\
h-vector sum & 0.446 & --- & --- & --- \\
SSR & --- & 76.4 & --- & --- \\
...
The OLS error-correction speed (0.063) is around 60\% higher than the
calibrated value (0.040), implying faster employment adjustment than the
prior assumed. The negative AR(2--4) coefficients likely reflect
quarterly seasonality absorption.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Employment: Dynamic Contributions}]{contrib_n.png}}
\caption{Employment: Dynamic Contributions}
\end{figure}
...
\subsubsection{Table 4.5.2: Consumption short-run PAC
coefficients}\label{table-4.5.2-consumption-short-run-pac-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
b0\_c (EC) & 0.060 & 0.069 & \textbf{0.0720} {[}0.0321, 0.1119{]} \\
b1\_c (AR1) & 0.149 & 0.046 & \textbf{0.0345} {[}0.0049, 0.0650{]} \\
b2\_c (rate gap) & -0.020 & -0.553 & \textbf{-0.3570} {[}-0.6366,
-0.0657{]} \\
b3\_c (output gap/HtM) & 0.139 & 0.018 & \textbf{0.0202} {[}-0.0651,
0.1011{]} \\
alpha\_c\_r · pv\_r\_lh\_gap (real-rate NPV channel) & -0.95 & --- &
calibrated \\
stderr eps\_c & --- & --- & \textbf{1.8134} {[}1.6179, 1.9858{]} \\
COVID crash & --- & -14.901 & --- \\
COVID bounce & --- & +6.388 & --- \\
h-vector sum & 0.678 & --- & --- \\
SSR & --- & 413.2 & --- \\
...
interest-rate sensitivity, consistent with Australia's variable-rate
mortgage structure; the Bayesian posterior pulls this toward
\textbf{−0.33} under prior regularisation, with the forward
\texttt{pv\_r\_lh\_gap} channel sharing rate-channel identification.
Second, the error-correction speed \(b^c_0 = 0.06\) implies a moderate
consumption-target adjustment speed. Third, the AR(1) coefficient is
negative without the COVID dummies; the dummies restore the expected
sign. h-vector amplification is 1.84×, the largest among the five PAC
equations.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Consumption: Dynamic Contributions}]{contrib_c.png}}
\caption{Consumption: Dynamic Contributions}
\end{figure}
...
\subsubsection{4.6 Business investment (FR-BDF Section
4.6.2)}\label{business-investment-fr-bdf-section-4.6.2}

\paragraph{4.6.1 Target equation (FR-BDF eq
63)}\label{target-equation-fr-bdf-eq-63}
...
\subsubsection{Table 4.6.1: Business investment target
coefficients}\label{table-4.6.1-business-investment-target-coefficients}
...
\paragraph{4.6.2 Short-run PAC equation (2nd-order, FR-BDF eq
64)}\label{short-run-pac-equation-2nd-order-fr-bdf-eq-64}
...
\subsubsection{Table 4.6.2: Business investment short-run PAC
coefficients}\label{table-4.6.2-business-investment-short-run-pac-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
b0\_ib (EC) & 0.030 & 0.018 & \textbf{0.0178} & {[}0.0050, 0.0291{]} \\
b1\_ib (AR1) & 0.181 & 0.107 & \textbf{0.0859} & {[}0.0231, 0.1526{]} \\
b2\_ib (AR2) & 0.100 & -0.046 & --- & --- \\
b3\_ib (accelerator) & 0.191 & 0.344 & \textbf{0.3314} & {[}0.1659,
0.4770{]} \\
stderr eps\_ib & --- & --- & \textbf{2.7677} & {[}2.5001, 3.0493{]} \\
COVID crash & --- & -4.382 & --- & --- \\
COVID bounce & --- & +2.978 & --- & --- \\
h-vector sum & 0.501 & --- & --- & --- \\
SSR & --- & 929.8 & --- & --- \\
...
\textbf{E-SAT auxiliaries} (wp736 Tables 4.6.11--12): two separate
equations --- output-gap auxiliary and user-cost auxiliary:
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Business Investment: Dynamic Contributions}]{contrib_ib.png}}
\caption{Business Investment: Dynamic Contributions}
\end{figure}

\emph{Figure 4.6.1: Historical dynamic-contribution decomposition of
Δlog i\_b (business investment growth) over 1994Q3--2024Q4. The
accelerator b3\_ib·yhat\_au (red) and the user-cost channel
−σ·pv\_rKB\_aux (brown) are the two most variable structural channels: a
positive output gap pulls investment up while a high user-cost PV pulls
it down, and they are negatively correlated through the cycle (rate cuts
in slumps support both channels in the same direction). The PAC
expectation contribution is small in level but timing-relevant ---
visibly leading the AR lags during the 2008-09 and 2020 turning points.}

\subsubsection{4.7 Household investment (FR-BDF Section
4.6.3)}\label{household-investment-fr-bdf-section-4.6.3}
...
\subsubsection{Table 4.7.2: Household investment short-run PAC
coefficients}\label{table-4.7.2-household-investment-short-run-pac-coefficients}
...
\bottomrule\noalign{}
\endlastfoot
b0\_ih (EC) & 0.049 & 0.028 & \textbf{0.0298} & {[}0.0064, 0.0514{]} \\
b1\_ih (AR1) & 0.210 & 0.111 & \textbf{0.1065} & {[}0.0326, 0.1753{]} \\
b2\_ih (AR2) & 0.080 & -0.032 & --- & --- \\
b3\_ih (output gap) & 0.120 & 0.231 & \textbf{0.2339} & {[}0.0747,
0.3983{]} \\
b4\_ih (rate gap) & dropped & --- & --- & --- \\
b\_ph\_ih (house prices) & 0.0099 (IV, fixed) & --- & --- & s.e. 0.075;
F=432, T=115 \\
stderr eps\_ih & --- & --- & \textbf{2.1051} & {[}0.4595, 4.7087{]} \\
COVID crash & --- & -5.558 & --- & --- \\
COVID bounce & --- & +2.603 & --- & --- \\
h-vector sum & 0.569 & --- & --- & --- \\
SSR & --- & 957.2 & --- & --- \\
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Housing Investment: Dynamic Contributions}]{contrib_ih.png}}
\caption{Housing Investment: Dynamic Contributions}
\end{figure}
...
sensible AU values and given Normal priors centred there (Section 5.4,
Table 5.6). A bivariate Engle-Granger style OLS of log volumes on log
...
\subsubsection{4.9 Demand deflators (FR-BDF Section
4.7)}\label{demand-deflators-fr-bdf-section-4.7}

All demand deflators follow ECM equations tracking the VA price with
partial pass-through, anchored to the long-run inflation target. General
form (eq. 36):
...
FR-BDF-style Hybrid \textgreater{} VAR amplification documented in §6.2.
...
gap formulation; §6.1.1 validates BGP stability under off-SS impulses.
...
forecasting (Section 6.4) and pseudo-real-time recursive forecast
(Section 5.5) take the US bloc, foreign demand, and commodity-price
paths as external assumptions while letting domestic policy and demand
respond endogenously. The exogenisable variables are \texttt{yhat\_us},
...
In each case the imported parameter is held fixed during MCMC; the
Bayesian estimation jointly identifies only the 28 parameters in Table
5.6. The audit (§6) flags these as ``imported, not AU-estimated'' for
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Observable Time Series}]{data_observables.png}}
\caption{Observable Time Series}
\end{figure}
...
\subsubsection{5.2 E-SAT Bayesian
...
\item
  ...
  full-system Bayesian estimate (Section 5.4) refines this to 0.329,
  consistent with moderate own-lag persistence in a standard NK wage
  Phillips curve.
\item
  CPI passthrough (gamma\_w): 0.770 in E-SAT, refined to 0.138 in the
  full-system Bayesian (Section 5.4) --- moderate contemporaneous CPI
  passthrough.
\item
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Posterior densities --- 28 parameters}]{identification_prior_posterior.png}}
\caption{Posterior densities --- 28 parameters}
\end{figure}

\emph{Figure 5.2: Prior vs posterior densities for E-SAT core
parameters.}

\subsubsection{5.3 PAC structural estimation --- intermediate
methodology
...
single-equation OLS with \texttt{calib\_smoother}-derived auxiliary
targets) is an \emph{intermediate methodological audit} that
\textbf{does not feed the final calibration}. The authoritative PAC
parameter values used throughout the rest of the paper --- in the IRFs,
conditional forecasts, and three-regime experiments --- are the
posterior means from the full-system Bayesian Metropolis-Hastings
estimation reported in §5.4 (Table 5.6), which uses raw observables and
does \emph{not} pre-smooth the data. FR-BDF wp736 does not use
Kalman-smoothed targets either; we report the iterative-OLS approach
here only as a robustness check on the MCMC numbers (the two approaches
agree at the 1-σ level for all five PAC equations) and as documentation
of the methodological path that led to the final specification.
...
\subsubsection{5.4 Bayesian full-system
estimation}\label{bayesian-full-system-estimation}

The authoritative AUSPAC calibration (Table 5.6) is produced by joint
Bayesian estimation of the simulation model using Dynare's
...
Blanchard-Kahn satisfaction. The input data are \emph{raw} quarterly
observables (\texttt{estimation\_data.mat}) --- no Kalman smoothing,
HP-filtering, or other pre-processing is applied to the observation
series. Dynare's Kalman filter is used internally to compute the
posterior density given a parameter draw, which is standard for Bayesian
DSGE estimation and does not constitute data smoothing in the FR-BDF
sense. This matches the wp736 methodology, which estimates PAC equations
on observable proxies without filtering or smoothing.

\subsubsection{Table 5.6: Bayesian posterior (28
parameters)}\label{table-5.6-bayesian-posterior-28-parameters}

The full-system Bayesian estimation jointly identifies 19 PAC and
wage-block parameters together with 9 shock standard deviations,
conditional on the calibrated supply, deflator, financial, trade, and
UIP-NPV parameters quoted elsewhere in §4. The estimation is two-stage:
csminwel mode search followed by Metropolis-Hastings MCMC (20,000 draws
across two chains, \textasciitilde51 min wall time). Posterior modes are
within ±5\% of posterior means for all coefficients, indicating
well-shaped log-likelihood surfaces.
...
The log marginal density is \textbf{Laplace = −781.05} and
...
\subsubsection{5.5 Pseudo-real-time recursive forecast
evaluation}\label{pseudo-real-time-recursive-forecast-evaluation}

Holding parameters at the posterior mode (no recursive re-estimation),
the model is re-run over 24 expanding-window origins from 2018Q1 to
2023Q4. At each origin t* the Kalman smoother yields the in-sample
state, and Dynare's \texttt{forecast=8} block produces conditional 1Q to
8Q-ahead forecasts of all observables under zero structural shocks.
Driver: \href{forecast_eval.m}{\texttt{forecast\_eval.m}}.

\subsubsection{Table 5.8: Pseudo-real-time recursive RMSE --- 24 origins
(2018Q1--2023Q4)}\label{table-5.8-pseudo-real-time-recursive-rmse-24-origins-2018q12023q4}
...
\bottomrule\noalign{}
\endlastfoot
yhat\_au & 1.819 & 2.335 & 2.304 & 2.338 \\
pi\_au & 0.782 & 0.805 & 0.810 & 0.892 \\
i\_au & 0.099 & 0.181 & 0.322 & 0.495 \\
yhat\_us & 2.163 & 2.396 & 2.410 & 2.405 \\
pi\_us & 0.448 & 0.530 & 0.673 & 0.772 \\
pi\_w & 0.199 & 0.254 & 0.236 & 0.338 \\
dln\_c & 3.911 & 3.897 & 3.873 & 4.219 \\
dln\_ib & 2.586 & 2.722 & 2.377 & 2.369 \\
i\_10y & 0.117 & 0.173 & 0.264 & 0.417 \\
\end{longtable}
}

Three patterns emerge. First, cash-rate and 10Y yield RMSEs grow
approximately linearly with horizon, consistent with a near-random-walk
process --- the h=8 yield RMSE is 42 bp, plausible for two-year-ahead
forecasts. Second, output-gap and inflation RMSEs are largely flat
across horizons because the model converges to steady state quickly
under zero-shock projection; the floor RMSE reflects the standard
deviations of the observable data themselves. Third, consumption growth
(\texttt{dln\_c}) and business-investment growth (\texttt{dln\_ib})
RMSEs remain elevated because the evaluation window contains the 2020
COVID quarter, where \texttt{dln\_c} reached -16\% --- the model
attributes this entirely to the eps\_c residual but cannot forecast it.
Excluding the COVID quarter would roughly halve the \texttt{dln\_c} and
\texttt{dln\_ib} RMSEs.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Recursive forecast paths}]{forecast_eval_paths.png}}
\caption{Recursive forecast paths}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pseudo-real-time RMSE by horizon}]{forecast_eval_rmse.png}}
\caption{Pseudo-real-time RMSE by horizon}
\end{figure}
...
\subsection{6. Model Properties}\label{model-properties}

\begin{quote}
\textbf{Scope note.} All IRFs and figures in this section are computed
at the Bayesian posterior mean of Table 5.6 under the CES calibration of
Table 4.2.1 and the trade ECM of §4.8. Peaks and troughs quoted in the
text are read from the saved IRF binaries; the corresponding figures are
reproduced from the same artefacts.
\end{quote}

\subsubsection{6.1 Steady state}\label{steady-state}
...
\paragraph{6.1.1 Long-run convergence to balanced growth
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Long-run convergence to balanced growth path}]{long_run_convergence_proxy.png}}
\caption{Long-run convergence to balanced growth path}
\end{figure}
...
\subsubsection{6.2 Monetary policy
...
\paragraph{6.2.1 Impulse responses}\label{impulse-responses}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Monetary policy IRFs}]{three_regime_monetary_irf.png}}
...
tightening)}\label{table-6.3-peak-irf-100bp-annualized-monetary-tightening}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Real GDP (ln\_Q)} & \textbf{−0.217\%} & Q7 \\
Output gap (yhat\_au) & \textbf{−0.137\%} & Q7 \\
CPI inflation (y/y) & \textbf{−0.099 pp} & Q11 \\
VA price inflation (y/y) & −0.010 pp & Q11 \\
Consumption growth (Δlog) & \textbf{−0.087\%} & Q2 \\
...
Exchange rate (s\_gap, \%) & \textbf{−0.971\%} & Q8 \\
10Y yield (annualised) & \textbf{+0.407 pp} & Q1 \\
\end{longtable}
}

\paragraph{6.2.2 Channel-by-channel
walkthrough}\label{channel-by-channel-walkthrough}

The aggregate \textbf{−0.22\% real GDP} response (Q7), \textbf{−0.14\%
output gap} (Q7), and \textbf{−0.10 pp y/y CPI inflation} decline (Q11)
decompose across six transmission channels documented in Table 2.2.

\textbf{(i) Term structure and WACC.} The 10Y yield rises \textbf{+0.41
...
accelerator \(b_3^{ib} = 0.32\) via the depressed output gap. The
combined effect is a \textbf{−0.05\%} peak in \(\Delta \ln I^B\) at Q7.
...
Net peak in growth: \(\Delta \ln I^H = -0.035\%\) at Q7. The level
...
immediately. Result: s\_gap peaks at \textbf{−0.97\% at Q8}. Through
...
\textbf{(v) Permanent income, real-rate NPV, and consumption.} The
consumption PAC equation (eq. 24) has four rate channels: the direct
term \(b_2^c \cdot \tilde{i}_{t-1}\) with \(b_2^c = -0.33\) (posterior
mean); the surprise term \(b_{di}^c \cdot di\_gap\) with
\(b_{di}^c = -0.70\) (Bayesian-regularised); the forward-NPV
real-lending-rate channel \(\alpha_{c,r} \cdot pv_{r,lh,gap}\) with
\(\alpha_{c,r} = -0.95\) (§4.5.2); and the permanent-income channel via
\(pv_{c,aux}\), which discounts future income at \(\beta_c = 0.95\). The
combined peak consumption-growth response is \textbf{−0.087\% at Q2} ---
the fastest demand response in the model. The \texttt{pv\_r\_lh\_gap}
channel is the key amplification mechanism: forward-looking agents see
both a higher impact 10Y yield (which lifts the mortgage rate via
\(\rho_{lh}\)) and a persistent forward path of elevated real lending
rates (the policy-rate gap mean-reverts slowly under
\(\lambda_i = 0.96\)), and cut consumption sharply on impact.

\textbf{(vi) Wage--price spiral.} Wage inflation responds negatively,
with peak −0.043 pp y/y at Q15, driven by the CPI-passthrough channel
...
a small reinforcing negative. VA-price inflation falls \textbf{−0.010 pp
y/y at Q11}, reflecting the ULC pass-through with
\(\gamma_{ulc} = 0.295\) modulated by the FPF target equation (eq. 14).

The channels together transmit monetary policy through a balanced mix of
cost-of-capital, exchange-rate, mortgage-rate, and expectations effects.
AU's variable-rate mortgages, the strong CPI-indexation wage Phillips,
and the forward-NPV real-rate consumption channel shape the relative
magnitudes --- most notably the large housing-investment response and
the rapid (Q2) consumption-growth peak via the forward 10Y yield, UIP
NPV, and \(pv_{r,lh,gap}\) channels.

\paragraph{6.2.4 Comparison with the RBA's model
...
\textbf{AU-PAC (hybrid, this paper)} & \textbf{−0.28\%} & \textbf{Q7} &
\textbf{−0.037} \\
\end{longtable}
}
...
\item
  ...
  AU).
\item
  \textbf{Estimated PAC coefficients are small in AU data.} The Bayesian
  posteriors (Table 5.6) give error-correction speeds in the 0.02--0.06
  range, AR coefficients in the 0.09--0.33 range, and accelerator
  coefficients (b3\_ib = 0.33, b3\_ih = 0.23, b5\_n ≈ 0) at or below
  their priors. PAC smoothness deliberately dampens responses relative
  to DSGE / VAR architectures.
\item
...
years'' (Q4--Q8). AU-PAC's output-gap peak sits at Q7 --- squarely in
the middle of this range --- and the real-GDP trough also comes at Q7.
The peak response is comparable to MARTIN/DINGO's short-horizon timing;
the cumulative capital decay produces a longer right tail than Beckers
VAR.
...
RBA suite: AUD appreciation peak −0.97\% at Q8. With ρ\_s = 0.775
(half-life ≈ 3 quarters) AU-PAC's mean-reversion to PPP is faster than
typical estimates, but the impact magnitude --- achieved on Q1--Q8
through the \texttt{pv\_i\_uip} forward NPV --- is comparable to
MARTIN/DINGO.
...
saver responses partially offset. AU-PAC's direct rate-change
consumption channel \(b_{di,c} = -0.70\) is Bayesian-regularised because
AU-data IV without external monetary-surprise data gave wrong-signed
estimates. In the model \(b_{di,c}\) contributes only a small fraction
of the consumption response, which is dominated by the \(b_2^c\)
interest-rate-level channel --- consistent with the RBA finding.

\textbf{Verdict.} AU-PAC produces the smallest peak response to a 100 bp
tightening among comparable Australian models (−0.22\% real GDP vs
Murphy's −0.30\% and the RBA workhorses' −0.45\% to −1.5\%). The peak
timing (Q7 for both GDP and the output gap) sits inside the MARTIN/DINGO
short-horizon range. The smaller AU-PAC peak magnitude reflects
deliberate features of the PAC architecture: adjustment-cost frictions
economically motivated by Tinsley (1993) and the FRB/US tradition, and
Bayesian estimation on 30 years of AU data that resists over-fitting.
AU-PAC therefore sits within the robust-policymaking spirit of
``diversity supports more robust policymaking'' (Mulqueeney et al.~2025,
p.~11) rather than aiming to match the average response. Two refinements
would narrow the gap: external high-frequency RBA OIS-surprise data for
\(b_{di,c}\) identification, and adopting the broader FR-BDF 2026
financial-block extensions (NFC accelerator, DSR-based mortgage block)
to amplify the cost-of-capital channel.

\subsubsection{6.3 Impulse responses to other
...
\paragraph{6.3.1 Monetary policy shock (eps\_i) --- 100bp
annualized}\label{monetary-policy-shock-eps_i-100bp-annualized}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Monetary policy shock}]{irf_eps_i.png}}
\caption{Monetary policy shock}
\end{figure}

\emph{Figure 6.3: 100bp annualized monetary policy tightening (scale =
0.25 / σ\_i = 0.25/0.111 ≈ 2.25).}

Consumption growth (−0.087\% peak at Q2) is the most immediately
responsive component, reflecting the level rate channel
(\(b_2^c = -0.33\) posterior mean) and the forward-NPV real-lending-rate
channel (\(\alpha_{c,r} = -0.95\) on \(pv_{r,lh,gap}\), §4.5.2).
Business-investment growth (−0.046\% at Q7) and housing-investment
growth (−0.035\% at Q7) follow with the lagged adjustment of the PAC
equations. The ln\_IH level continues to decline through the 40-quarter
horizon (long-tailed housing-investment response, consistent with AU's
outsized construction cycle). The exchange rate appreciates via UIP
(s\_gap −0.97\% at Q8). Real GDP (\(\ln Q\)) reaches its trough at
\textbf{−0.22\% at Q7}.

\paragraph{6.3.2 Foreign demand shock (eps\_q\_us) --- 1pp US output
gap}\label{foreign-demand-shock-eps_q_us-1pp-us-output-gap}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Foreign demand shock}]{irf_eps_q_us.png}}
\caption{Foreign demand shock}
\end{figure}

\emph{Figure 6.4: 1pp US output-gap shock (scale = 1.0 / σ\_q\_us =
1.0/1.138 = 0.879). Peak AU output gap +0.42\% at Q4, peak
housing-investment growth +0.19\% at Q5, peak export growth +0.35\% at
Q2 (foreign-demand channel now identified through the proper trade ECM).
The new specification routes more of the foreign-demand impulse through
direct trade (β\_x = 1.2 LR pull) and less through the housing channel
than the previous degenerate-trade calibration suggested.}

\paragraph{6.3.3 Government spending shock (eps\_g) --- 1\% of
GDP}\label{government-spending-shock-eps_g-1-of-gdp}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Government spending shock}]{irf_eps_g.png}}
\caption{Government spending shock}
\end{figure}

\emph{Figure 6.5: 1\% of GDP government spending shock (scale = 1.0 /
σ\_g = 1.0/0.3 = 3.333). Output-gap multiplier peaks at +0.18\% at Q4
--- well below 1 --- consistent with substantial crowding-out via the
exchange-rate channel under floating regime and forward-looking
financial agents in the hybrid setup. Imports rise +0.06\% at Q6 as
domestic demand pulls in foreign goods through the proper-ECM
income-elasticity channel (β\_m = 1.5).}

\paragraph{6.3.4 Commodity price shock (eps\_pcom) --- 10\%
increase}\label{commodity-price-shock-eps_pcom-10-increase}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Commodity price shock}]{irf_eps_pcom.png}}
\caption{Commodity price shock}
\end{figure}

\emph{Figure 6.6: 10\% commodity-price increase (scale = 10.0 / σ\_pcom
= 10.0/3.0 = 3.333). AU-specific propagation: peak output gap +0.18\% at
Q2, housing-investment growth +0.07\% at Q3, export growth +1.50\% at Q1
(commodity-price loading in \texttt{b4\_x}), import growth +0.19\% at Q2
via the income channel. The commodity-export economy benefits in real
...
\paragraph{6.3.5 Cost-push shock (eps\_pQ) --- 1pp VA-price
inflation}\label{cost-push-shock-eps_pq-1pp-va-price-inflation}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Cost-push shock}]{irf_eps_pQ.png}}
\caption{Cost-push shock}
\end{figure}

\emph{Figure 6.7: 1pp VA-price inflation cost-push shock (scale = 1.0 /
σ\_pQ ≈ 1.75). Cost-push transmission operates through both the
structural deflator channels in the E-SAT inflation equation (§4.4.0)
and the level price-block dynamics. The VA-price PAC equation
(eq\_piQ\_pac) absorbs ε\_pQ directly into π\_Q on impact (+0.57 qpp).
Through the structural channels in eq\_au\_phillips, π\_Q propagates
into the CPI inflation gap immediately (+0.12 qpp impact), the Taylor
rule begins tightening by Q5, and the output gap turns negative around
Q8.}
...
+1pp cost-push shock that is muted relative to FR-BDF: the output gap
turns mildly negative (−0.003\% at Q8, peak −0.008\% by Q12) but the GDP
level \(\ln Q\) remains modestly positive throughout the IRF horizon
(peak +0.20\% at Q2, declining to +0.06\% at Q20). The qualitative
cost-push transmission --- CPI rising on impact, the policy rate
tightening with a lag, demand contracting --- matches FR-BDF; the
quantitative magnitudes reflect the AU institutional features documented
above.

\paragraph{6.3.6 TFP shock (eps\_tfp) --- 1\% level
shift}\label{tfp-shock-eps_tfp-1-level-shift}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={TFP shock}]{irf_eps_tfp.png}}
\caption{TFP shock}
\end{figure}

\emph{Figure 6.8: 1 s.d. TFP shock (σ\_tfp = 0.2; scale = 1.0; hybrid
regime). Plotted at 1 s.d. rather than 1\% because rho\_tfp = 0.99 (near
unit root) makes the level response near-permanent and growing over the
IRF window --- scaling to 1\% would produce implausibly large level
shifts (50\%+ in ln\_N). At 1 s.d., output level (ln\_Q) rises +13\%
over 40 quarters; potential output (ln\_QN) tracks one-for-one (the
supply shock raises both by construction in the gap model), so the
output gap yhat\_au = ln\_Q − ln\_QN stays at zero. Employment level
rises \textasciitilde+10\% as the CES production function reallocates
inputs. Wage inflation picks up TFP via the productivity term
\((1-\lambda_w) \Delta \ln Prod\) in eq\_pi\_w with peak +0.3 qpp ---
the only structural channel from supply-side TFP into nominal dynamics.
Gap-variable panels (yhat\_au, dln\_n) and unrelated variables (piQ,
i\_10y) stay at floating-point-noise level by construction.}

\paragraph{6.3.7 Term premium shock (eps\_tp) --- 100bp
annualized}\label{term-premium-shock-eps_tp-100bp-annualized}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Term premium shock}]{irf_eps_tp.png}}
...
level falls -0.10\%, business investment level -0.08\%, housing
investment level -0.16\% over 40 quarters; consumption permanent income
(ln\_C\_star) tracks the consumption level closely. Output gap
(yhat\_au) and VA price inflation (piQ) stay at floating-point noise ---
the term-premium shock has no direct flow to the output-gap or short-run
inflation dynamics in this architecture. This is the same architectural
pattern that makes the APP-style experiment (Figure 6.14) deliver modest
level effects rather than dramatic gap responses.}

\paragraph{6.3.8 Output gap overview --- all shocks (policy-relevant
sizes)}\label{output-gap-overview-all-shocks-policy-relevant-sizes}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Output gap overview}]{irf_overview_output.png}}
...
scenario re-run under the current calibration is straightforward (see
\texttt{conditional\_forecast\_driver.m}).}

\subsubsection{6.5 Forward guidance
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Forward guidance puzzle test}]{forward_guidance_puzzle.png}}
\caption{Forward guidance puzzle test}
\end{figure}

\emph{Figure 6.13: Forward guidance puzzle test --- AU-PAC (solid) vs
standard NK (dashed). AU-PAC scales approximately linearly.}

\subsubsection{6.6 APP-style experiment --- 200bp persistent
term-premium
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={APP-style experiment}]{app_experiment_200bp.png}}
\caption{APP-style experiment}
\end{figure}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{The PAC framework is well identified on Australian data.}
  Iterative OLS and Bayesian estimation produce mutually consistent
  estimates across 67 parameters identified from Australian data.
  Error-correction speeds, AR dynamics, and output-gap sensitivities are
  in the expected ranges; full-system Bayesian estimation under the CES
  calibration of Table 4.2.1, the forward-NPV UIP
  (\(\beta_{uip} = 0.92\)), and the 11-observable specification gives
  log marginal density \textbf{Laplace = −781.05} and \textbf{Modified
  Harmonic Mean = −781.39}.
\item
  \textbf{The Australian wage Phillips curve has standard New Keynesian
  structure.} Estimated on the ABS Wage Price Index, γ\_w = 0.137 (90\%
  HPD {[}0.082, 0.191{]}), λ\_w = 0.337 (HPD {[}0.179, 0.514{]}), and
  κ\_w = 0.095 (HPD {[}0.023, 0.176{]}) --- moderate contemporaneous CPI
  passthrough, moderate own-lag persistence, and a statistically
  significant Phillips slope. The Fair Work Commission's institutional
  CPI indexation operates on roughly 15\% of the workforce (award rates
  plus enterprise-agreement floors); most AU wage growth is set by
  private bargaining responsive to the unemployment-gap PV. AU wages
  therefore adjust to labour-market slack via a Phillips-curve channel
  and to inflation via a moderate contemporaneous CPI coefficient plus
  the inflation-anchor weight (1 − λ\_w − γ\_w = 0.53).
\item
  ...
  coefficients. Two pulse dummies per equation resolve this cleanly.
\item
  \textbf{Monetary policy transmits through a balanced mix of channels
  with realistic AU-specific magnitudes.} A 100bp annualised cash-rate
  tightening produces a peak real-GDP fall of −0.22\% at Q7, an
  output-gap peak of −0.14\% at Q7, a CPI-inflation fall of 0.10 pp y/y
  at Q11, an AUD appreciation of 0.97\% at Q8 via the forward-NPV UIP,
  and a 10-year yield rise of 0.41 pp on impact via the term-structure
  NPV. The transmission combines cost-of-capital, exchange-rate,
  mortgage-rate, and expectations effects, with AU's variable-rate
  mortgage market and strong CPI-indexation wage Phillips shaping the
  relative magnitudes.
\item
...
\item
  \textbf{The consumption interest-rate channel is significant but
  uncertain in magnitude.} The Bayesian posterior for b2\_c = −0.330
  (HPD {[}−0.631, −0.047{]}) confirms nonzero interest-rate sensitivity
  in consumption. The direct rate-\emph{change} effect b\_di\_c is not
  identified from OLS on AU data alone (reverse causality) and remains a
  candidate for high-frequency-surprise IV estimation. The housing-price
  gap in housing investment b\_ph\_ih is small but positive (0.0099, IV
  F = 432) on the extended 1959Q3-onward housing-price series.
\end{enumerate}

\textbf{Comparison with the RBA model suite.} Section 6.2.4 benchmarks
AU-PAC against the four Australian models surveyed in Mulqueeney,
Ballantyne and Hambur (2025). AU-PAC's peak real-GDP response (−0.22\%
at Q40) is at the low end of the RBA suite --- comparable to Murphy's
−0.30\%, about half MARTIN's −0.45\%, and roughly 7× smaller than the
Beckers VAR's −1.5\%. The conservative response reflects three
deliberate features: PAC adjustment-cost frictions that distinguish
FRB/US-style models from VAR / DSGE alternatives; the FR-BDF 2026 CES
calibration with higher capital--labour substitutability that dampens
factor-price movements; and AU-specific Bayesian posteriors that resist
over-fitting. The IRF timing also differs: AU-PAC's GDP trough is at Q40
rather than within 2 years, reflecting the cumulative capital decay that
drags potential output down well after the demand-side gap closes.
AU-PAC therefore complements the RBA suite on the conservative end of
``diversity supports more robust policymaking'' (Mulqueeney et al.~2025,
p.~11).

\textbf{Remaining work.} Four extensions would sharpen the analysis:
high-frequency RBA OIS-surprise IV estimation for \texttt{b\_di\_c}
(Bishop--Tulip 2017; Beechey--Wright 2009); an energy / non-energy
import split along the lines of wp736 eqs 88--91 (less material for AU
as a net energy exporter, but adds rigour); a full historical shock
decomposition; and a systematic channel-decomposition exercise of the
type Mulqueeney et al.~(2025) report for MARTIN and DINGO, in which each
transmission channel is ``turned off'' in turn to isolate exchange-rate,
asset-price, savings / investment, and cash-flow contributions. Adopting
FR-BDF 2026's broader updates beyond the supply block --- particularly
the Dees et al.~(2022) NFC financial accelerator and the Bove et
al.~(2020) household debt-service-ratio block --- is the natural v3.0
direction.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{8. Australia-Specific
...
\subsubsection{A.2 VA Price Block (5
variables)}\label{a.2-va-price-block-5-variables}
...
piQ\_star & VA price target inflation & 0.625 \\
piQ\_star\_bar & VA price trend inflation & 0.625 \\
pQ\_gap & VA price gap & 0 \\
...
ln\_ib\_level, ln\_ih\_level, ln\_n\_level, pQ\_star\_level).
...
non-zero in the current Dynare 6.5 implementation (the companion matrix
...
\url{https://github.com/DavidAStephan/AUSPAC}.
...
Dynare 6.5's \texttt{identification} command requires analytic
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Posterior densities for 28 estimated parameters}]{identification_prior_posterior.png}}
\caption{Posterior densities for 28 estimated parameters}
\end{figure}
...
\url{https://gitlab.com/srecko/SemiStructDynareBasics}
...
used as the wage-inflation observable in Section 5.4.
