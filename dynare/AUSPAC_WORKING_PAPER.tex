...
cash-rate tightening produces a real GDP level that rises +0.478\% by
Q40 (supply-side ULC effect), an output-gap trough of −0.126\% at Q7,
CPI inflation falling −0.105 pp year-on-year at Q16, the AUD
appreciating 0.947\% at Q8 via a forward-looking UIP NPV channel, and
the 10-year yield rising 0.098 pp on impact. A cross-block finding
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Real GDP (ln\_Q)} & \textbf{+0.478\%} & Q40 \\
Output gap (yhat\_au) & \textbf{−0.126\%} & Q7 \\
CPI inflation (pi\_au, y/y) & \textbf{−0.105 pp} & Q16 \\
VA price inflation (piQ, y/y) & +0.041 pp & Q1 \\
CPI deflator inflation (pi\_c, y/y) & −0.133 pp & Q18 \\
Consumption growth (Δlog) & \textbf{−0.133\%} & Q2 \\
...
Exchange rate (s\_gap, \%) & \textbf{−0.947\%} & Q8 \\
10Y yield (annualised) & \textbf{+0.098 pp} & Q1 \\
\end{longtable}
}
...
The aggregate \textbf{+0.478\% real GDP} long-run level rise (Q40, from
supply-side ULC effect), \textbf{−0.126\% output gap} trough (Q7), and
\textbf{−0.105 pp y/y CPI inflation} decline (Q16) decompose across six
transmission channels documented in Table 2.2.

\textbf{(i) Term structure and WACC.} The 10Y yield rises \textbf{+0.098
...
\textbf{−0.101\%} peak in \(\Delta \ln I^B\) at Q6. The structural form
(coef = +1 on PV(Δq̂) + PV(Δq̄), coef = −σ on PV(Δlog r̂\_KB) + PV(Δlog
...
Net peak in growth: \(\Delta \ln I^H = -0.032\%\) at Q7. The level
...
immediately. Result: s\_gap peaks at \textbf{−0.947\% at Q8}. Through
...
combined peak consumption-growth response is \textbf{−0.133\% at Q2} ---
...
with peak −0.114 pp y/y at Q18, driven by the CPI-passthrough channel
...
+0.041 pp y/y on impact (Q1)}, reflecting the ULC cost-push channel:
...
deflator inflation (pi\_c) peaks at −0.133 pp y/y at Q18.
...
\textbf{AU-PAC (Phase T, this paper)} & \textbf{+0.478\%} (long-run
level rise) & \textbf{Q40} & \textbf{−0.105} \\
\end{longtable}
}
...
RBA suite: AUD appreciation peak −0.947\% at Q8. With ρ\_s = 0.775
...
\textbf{Verdict.} AU-PAC's output-gap trough (−0.126\% at Q7) is the
...
to −1.5\% in GDP terms). The real GDP level rises long-run (+0.478\% at
Q40) due to the supply-side ULC channel --- a structural feature not
present in pure-demand models. The output-gap peak timing (Q7) sits
...
Consumption growth (−0.133\% peak at Q2) is the most immediately
...
Business-investment growth (−0.101\% at Q6) and housing-investment
growth (−0.032\% at Q7) follow with the lagged adjustment of the PAC
...
(s\_gap −0.947\% at Q8). Real GDP (\(\ln Q\)) rises in the long run to
\textbf{+0.478\% at Q40} (supply-side ULC effect), while the output gap
troughs at \textbf{−0.126\% at Q7}.
...
\item
  ...
  tightening produces a real-GDP level rise of +0.478\% by Q40
  (supply-side ULC effect), an output-gap trough of −0.126\% at Q7, a
  CPI-inflation fall of −0.105 pp y/y at Q16, an AUD appreciation of
  0.947\% at Q8 via the forward-NPV UIP, and a 10-year yield rise of
  0.098 pp on impact via the term-structure NPV. The transmission
  combines cost-of-capital, exchange-rate, mortgage-rate, and
  expectations effects, with AU's variable-rate mortgage market and
  strong CPI-indexation wage Phillips shaping the relative magnitudes.
  Under the post-hybrid Option 1 BI calibration the BI channel is
  substantively larger than under the pre-hybrid AU-estimated parameters
  (BI ECM 5× faster, AR1 4× more inertial, accelerator 2× stronger); the
  qualitative shape is preserved, the quantitative magnitude through the
  BI channel is moderately amplified (§5.4, §7.2).
\item
...
and Hambur (2025). AU-PAC's output-gap trough (−0.126\% at Q7) is at the
low end of the RBA suite in demand-response terms, while the real-GDP
level rises long-run (+0.478\% at Q40) via the supply-side ULC channel.
The conservative response reflects three deliberate features: PAC
adjustment-cost frictions that distinguish FRB/US-style models from VAR
/ DSGE alternatives; the FR-BDF 2026 CES calibration with higher
