...
0.079, t=3.27) & \textbf{0.0519} & {[}0.0215, 0.0805{]} \\
β₁ (Δc lag 1) & 0.17 & −0.159 (SE 0.087, t=−1.83) hit clamp &
\textbf{0.0397} & {[}0.0051, 0.0731{]} \\
β₂ (rate gap) & --- & --- & \textbf{−0.5796} & {[}−0.8848, −0.2866{]} \\
β₃ (HtM / output gap) & 0.32 & −0.018 (SE 0.076, t=−0.23) &
\textbf{0.0285} & {[}−0.0543, 0.1134{]} \\
α₁ (PV(r\_LH gap), reduced-form coef) & −1.15 (from LR Eq 33) &
\textbf{−85.5} (SE 31.1, t=−2.75); reduced form α₁·χ\_AU = −0.81 & ---
(calibrated α\_c\_r = −0.95) & --- \\
β\_PAC (free coef on Δȳ lag) & (free, not tabled) & \textbf{+1.47} (SE
0.70, t=2.12) & \textbf{0.8263} & {[}0.3315, 1.3595{]} \\
...
\subsubsection{Table 5.6: Bayesian posterior (25 estimated parameters
--- hybrid calibration, BI block calibrated from
wp1044)}\label{table-5.6-bayesian-posterior-25-estimated-parameters-hybrid-calibration-bi-block-calibrated-from-wp1044}

\textbf{Log marginal data density}: Laplace = \textbf{−696.06}, MHM =
\textbf{−697.36}. Improvement vs Round 1.2 baseline: \textbf{+88.4 nats
MHM} (−785.80 → −697.36). The largest single-phase gain in the project's
Phase Q--L2 trajectory. Estimated on 20k×2-chain MH, MATLAB R2026a +
Dynare 7.0 (native ARM, parallel pool with 6 workers), total wall time
15 min. Three BI parameters (b0\_ib, b1\_ib, b3\_ib) are removed from
\texttt{estimated\_params} per the Option 1 hybrid calibration of §3.6 +
§4.6.2.

\begin{quote}
\textbf{Note}: the pre-Phase-L2 MCMC (Phase Y) estimated 28 parameters
including 3 BI params. The current hybrid MCMC estimates 25 (BI
calibrated). The +88 nats LMD improvement comes from both the dimension
reduction (\textasciitilde4.5 nats mechanical from losing 3 free params)
and the genuinely better BI block dynamics under wp1044 imported
coefficients (\textasciitilde84 nats genuine fit improvement).
\end{quote}
