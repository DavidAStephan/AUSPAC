// --+ options: stochastic,json=compute +--
// =========================================================================
// aux_pQ.mod — Phase L2.A regeneration aux file for VA-price PAC equation
//
// PURPOSE (estimation only, never simulated directly): use Dynare's
// var_model + pac_model infrastructure to compute the closed-form
// expectation formula for pac_expectation(pac_pQ). The h-coefficients
// are then patched into the simulation au_pac.mod.
//
// Reconstructed 2026-05-28 from the pre-cleanup template (commit 7995ce7^)
// with parameter values resynced to the post-L2-OLS / Phase L2.A
// production calibration in au_pac.mod.
// =========================================================================

var
    yhat_au         pi_au_gap       i_gap           u_gap
    yhat_us         pi_us_gap
    ibar            pibar_au        pibar_us
    piQ             pi_m            dln_pcom
    pi_w_gap
    tau_GST_gap
    piQ_hat
    pQ_level
;

varexo
    eps_q           eps_i           eps_pi          eps_q_us
    eps_pi_us       eps_ibar        eps_pibar_au    eps_pibar_us
    eps_u_gap       eps_piQ         eps_pi_m        eps_pcom
    eps_pi_w        eps_tau_GST     eps_var_pQ      eps_pQ
;

parameters
    lambda_q        sigma_q         delta
    lambda_i        alpha_i         beta_i
    lambda_pi       kappa_pi
    lambda_q_us     lambda_pi_us    kappa_pi_us
    lambda_ibar     lambda_pibar    lambda_pibar_us
    rho_u_gap       okun_coeff
    alpha_pc        beta_pc_m       gamma_oil
    i_ss            pi_ss_au        pi_ss_us
    rho_pQ_aux      a_pQ_y          a_pQ_i          a_pQ_pi         a_pQ_u          a_pQ_w
    rho_piQ
    rho_pm          rho_pcom
    rho_pi_w_gap    a_w_pi          a_w_u
    rho_tau_GST     a_pQ_GST
    b0_pQ           b1_pQ           b2_pQ
    beta_pac
;

// -----------------------------------------------------------------------
// Calibration — synced to production au_pac.mod (post-L2 OLS + Phase L2.A).
// E-SAT structural calibrations that survived L2 OLS (lambda_q, sigma_q,
// lambda_i, alpha_i, beta_i, lambda_q_us, lambda_pi_us, kappa_pi_us,
// trend persistences, i_ss, pi_ss_*).
// -----------------------------------------------------------------------
lambda_q        = 0.6959;       sigma_q         = 0.0648;       delta           = 0.1989;
lambda_i        = 0.9576;       alpha_i         = 0.3001;       beta_i          = 0.0837;
lambda_q_us     = 0.8057;       lambda_pi_us    = 0.6529;       kappa_pi_us     = 0.0131;
lambda_ibar     = 0.985;        lambda_pibar    = 0.93;         lambda_pibar_us = 0.93;
i_ss            = 1.0491;       pi_ss_au        = 0.625;        pi_ss_us        = 0.5;

// L2 OLS overrides (au_pac.mod lines 723-724, 1922-1929) — AU point estimates
lambda_pi       =  0.2588;      // AU OLS, t=2.76 (was 0.2902)
kappa_pi        = -0.0336;      // AU OLS, insig + wrong-signed (was 0.0374)
alpha_pc        =  0.0006;      // contemp VA→CPI (AU OLS; was 0.17)
beta_pc_m       = -0.0042;      // import→CPI passthrough (AU OLS; was 0.10)
gamma_oil       =  0.03;        // unchanged (no AU dln_pcom regressor in OLS)
rho_u_gap       =  0.946;       // u_gap persistence (L2 OLS; was 0.94)
okun_coeff      = -0.13;        // Okun's law (L2 OLS; was -0.33)

// Auxiliary regression for piQ_hat (Phase W posteriors, locked in production)
rho_pQ_aux      =  0.334;
a_pQ_y          =  0.043;
a_pQ_i          = -0.021;
a_pQ_pi         =  0.007;
a_pQ_u          = -0.021;
a_pQ_w          =  0.4367;      // wage→piQ_hat (AU OLS Phase U)

// AR(1) reduced forms for VAR companion matrix purposes (agents' beliefs;
// the structural dynamics of these variables live in au_pac.mod)
rho_piQ         =  0.85;        // piQ AR(1) — agents' belief (no production override)
rho_pm          =  0.2230;      // AU OLS (was 0.28)
rho_pcom        =  0.42;        // unchanged

// Wage Phillips reduced form (agents' belief — no production override)
rho_pi_w_gap    =  0.21;        a_w_pi          = 0.35;         a_w_u           = -0.05;

// GST tax-gap process (Round 6 calibration)
rho_tau_GST     =  0.90;        a_pQ_GST        = 0.05;

// VA-price PAC short-run coefficients (L2 OLS post-Phase L2)
b0_pQ           =  0.2580;      // ECM speed (L2 OLS; was 0.0294)
b1_pQ           =  0.3039;      // piQ lag (L2 OLS; was 0.2784)
b2_pQ           = -0.0760;      // yhat_au contemp (L2 OLS, wrong-signed insig; was 0.0022)

beta_pac        =  0.98;

// -----------------------------------------------------------------------
// var_model declaration: equation order defines the companion-matrix Φ
// row/column ordering. Must match the state order assumed by
// au_pac.mod's pac_expectation_pac_pQ definition.
// -----------------------------------------------------------------------
var_model(model_name = esat_pQ,
    eqtags = [
        'var_yhat_au', 'var_i_gap', 'var_pi_au_gap', 'var_u_gap',
        'var_yhat_us', 'var_pi_us_gap',
        'var_ibar', 'var_pibar_au', 'var_pibar_us',
        'var_piQ', 'var_pi_m', 'var_dln_pcom',
        'var_pi_w_gap',
        'var_tau_GST_gap',
        'var_piQ_hat'
    ]);

pac_model(auxiliary_model_name = esat_pQ, discount = beta_pac, model_name = pac_pQ);

model;

    // ---------------------------------------------------------------
    // E-SAT core equations (pure-VAR form, lagged-only RHS)
    // ---------------------------------------------------------------
    [name = 'var_yhat_au']
    yhat_au = lambda_q*yhat_au(-1) - sigma_q*(i_gap(-1) - pi_au_gap(-1)) + delta*yhat_us(-1) + eps_q;

    [name = 'var_i_gap']
    i_gap = lambda_i*i_gap(-1) + (1-lambda_i)*(alpha_i*pi_au_gap(-1) + beta_i*yhat_au(-1)) + eps_i;

    [name = 'var_pi_au_gap']
    pi_au_gap = lambda_pi*pi_au_gap(-1) + kappa_pi*yhat_au(-1) + alpha_pc*(piQ(-1) - pibar_au(-1)) + beta_pc_m*(pi_m(-1) - pibar_au(-1)) + gamma_oil*dln_pcom(-1) + eps_pi;

    [name = 'var_u_gap']
    u_gap = rho_u_gap*u_gap(-1) + okun_coeff*yhat_au(-1) + eps_u_gap;

    [name = 'var_yhat_us']
    yhat_us = lambda_q_us*yhat_us(-1) + eps_q_us;

    [name = 'var_pi_us_gap']
    pi_us_gap = lambda_pi_us*pi_us_gap(-1) + kappa_pi_us*yhat_us(-1) + eps_pi_us;

    [name = 'var_ibar']
    ibar = lambda_ibar*ibar(-1) + (1-lambda_ibar)*i_ss + eps_ibar;

    [name = 'var_pibar_au']
    pibar_au = lambda_pibar*pibar_au(-1) + (1-lambda_pibar)*pi_ss_au + eps_pibar_au;

    [name = 'var_pibar_us']
    pibar_us = lambda_pibar_us*pibar_us(-1) + (1-lambda_pibar_us)*pi_ss_us + eps_pibar_us;

    // Structural states reduced to AR(1) for var_model purposes
    [name = 'var_piQ']
    piQ = rho_piQ*piQ(-1) + (1-rho_piQ)*pi_ss_au + eps_piQ;

    [name = 'var_pi_m']
    pi_m = rho_pm*pi_m(-1) + (1-rho_pm)*pi_ss_au + eps_pi_m;

    [name = 'var_dln_pcom']
    dln_pcom = rho_pcom*dln_pcom(-1) + eps_pcom;

    // Wage inflation gap reduced form (agents' belief)
    [name = 'var_pi_w_gap']
    pi_w_gap = rho_pi_w_gap*pi_w_gap(-1) + a_w_pi*pi_au_gap(-1) + a_w_u*u_gap(-1) + eps_pi_w;

    // GST tax-gap reduced form
    [name = 'var_tau_GST_gap']
    tau_GST_gap = rho_tau_GST*tau_GST_gap(-1) + eps_tau_GST;

    // piQ_hat auxiliary projection (the trend_component target for the PAC)
    [name = 'var_piQ_hat']
    piQ_hat = rho_pQ_aux*piQ_hat(-1) + a_pQ_y*yhat_au(-1) + a_pQ_i*i_gap(-1) + a_pQ_pi*pi_au_gap(-1) + a_pQ_u*u_gap(-1) + a_pQ_w*pi_w_gap(-1) + a_pQ_GST*tau_GST_gap(-1) + eps_var_pQ;

    // VA-price PAC equation — this is what pac.print expands
    [name = 'eq_piQ_pac']
    diff(pQ_level) = b0_pQ*(piQ_hat(-1) - pQ_level(-1))
                   + b1_pQ*diff(pQ_level(-1))
                   + pac_expectation(pac_pQ)
                   + b2_pQ*yhat_au
                   + eps_pQ;

end;

shocks;
    var eps_q;          stderr 0.527;
    var eps_i;          stderr 0.110;
    var eps_pi;         stderr 0.590;
    var eps_q_us;       stderr 1.138;
    var eps_pi_us;      stderr 0.319;
    var eps_ibar;       stderr 0.01;
    var eps_pibar_au;   stderr 0.01;
    var eps_pibar_us;   stderr 0.01;
    var eps_u_gap;      stderr 0.05;
    var eps_piQ;        stderr 0.571;
    var eps_pi_m;       stderr 0.5;
    var eps_pcom;       stderr 3.0;
    var eps_pi_w;       stderr 0.1407;
    var eps_tau_GST;    stderr 0.10;
    var eps_var_pQ;     stderr 0.01;
    var eps_pQ;         stderr 0.571;
end;

pac.initialize('pac_pQ');
pac.update.expectation('pac_pQ');
pac.print('pac_pQ', 'eq_piQ_pac');
