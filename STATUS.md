...
**Estimation methodology**: equation-by-equation OLS/calibration following wp1044 (Dubois et al. 2026). No joint Bayesian MCMC in the production parameter pipeline — matching the FR-BDF estimation methodology throughout. PAC block parameters from Phase L2 iterative-OLS (`data/pac_blocks/results_*.mat`); non-PAC parameters from single-equation OLS or wp1044 calibration references. The Bayesian MCMC (`au_pac_bayesian.mod`) is available as a diagnostic tool but does not drive production parameters.
**Diagnostic MCMC (for reference)**: Laplace = −662.27, MHM = −665.51 (full-structural-model MCMC, 2026-05-28). Cumulative Phase Q → MCMC: +136.8 nats. Not used for production parameters.
