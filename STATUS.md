...
**Headline metric (current cache)**: Laplace LMD = **−696.06**, MHM LMD = **−697.36** — fresh 20k×2-chain MCMC under Phase L2 P1c hybrid calibration completed 2026-05-27. **+88.4 nats MHM improvement** vs the Round 1.2 baseline (−785.80). The largest single-phase gain in the project's Phase Q–L2 trajectory. Cumulative Phase Q → current MHM improvement: **+104.9 nats** (from −802.27 to −697.36). MATLAB R2026a + Dynare 7.0 (native ARM, parallel pool, 15 min wall time). Chains cached in `dynare/au_pac_bayesian/metropolis/`.
