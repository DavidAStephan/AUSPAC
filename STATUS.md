...
**Headline metric**: Laplace LMD = **−779.30**, MHM LMD = **−780.36** — both restored 2026-05-20 from a fresh 20k×2-chain MCMC (Laplace within 0.02 nats and MHM within 0.1 nats of pre-cleanup values; MHM difference is within typical stochastic noise of the modified-harmonic-mean estimator). Chains and mode file cached in `dynare/au_pac_bayesian/`; estimation block defaults to `mode_compute=0` + `load_mh_file` for cheap reload (~30 sec). Cumulative Phase Q → Phase Y MHM improvement: **+21.80 nats** (built into the model). Rounds 4–8 model extensions added 2026-05-20 do not affect the likelihood (no new `varobs` / `estimated_params`), so the cached MCMC remains valid for the extended model.
