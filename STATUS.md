...
**Version**: v3.1.2 (Rounds 4–8 expectation consolidation merged 2026-05-21, PR #6)
**Architecture**: Phase T policy-function PAC expectations (FR-BDF wp1044 §2.2 / Adjemian-Brayton-Zimic), with Phases W/X/Y architectural cleanup completed, plus Round 1.1 HICP-style reporting block and 2026-05-21 consolidation that wired the Round 5–6 shocks into per-block PAC `var_model` companion matrices.
**Headline metric**: Laplace LMD = **−779.29**, MHM LMD = **−780.10** — fresh 20k×2-chain MCMC under consolidated expectations completed 2026-05-21 (vs cached pre-consolidation Laplace −779.30 / MHM −780.36; both within typical stochastic noise of the MHM estimator). Chains and mode file cached in `dynare/au_pac_bayesian/`; estimation block defaults to `mode_compute=0` + `load_mh_file` for cheap reload (~30 sec). Cumulative Phase Q → Phase Y MHM improvement: **+22.17 nats** (built into the model). Likelihood is provably preserved by the consolidation: the four new shocks ($\varepsilon^{pop,bar}, \varepsilon^{\tau,GST}, \varepsilon^{\tau,PAYG}, \varepsilon^{\tau,CIT}$) are not in `varobs` and have zero historical realisations, so the new $h_{pac,X,Y}$ terms in each `pac_expectation_pac_X` identity vanish in the historical sample.
