...
**Version**: v3.1.4 (Round 1.2 follow-up exhaustive — Option 4 stim-NaN, TVD, FR-BDF, Option α model-carried trend; 2026-05-23)
**Architecture**: Phase T policy-function PAC expectations (FR-BDF wp1044 §2.2 / Adjemian-Brayton-Zimic), with Phases W/X/Y architectural cleanup completed, plus Round 1.1 HICP-style reporting block, 2026-05-21 Rounds 4-8 consolidation, 2026-05-22 Round 1.2 hand-to-mouth income channel, and 2026-05-23 Option α: FR-BDF-faithful trend treatment (constant BGP drift `g_bar_C = g_bar_IB = 0.498% qoq` in `dln_c_star_bar` / `dln_ib_star_bar`; observable matching via new `dln_C_obs` / `dln_IB_obs` total-growth variables; raw `dln_c` / `dln_ib` data — no demeaning).
**Headline metric (current cache, Option α)**: MHM LMD = **−1101.87** on the 10-observable Round 1.2 follow-up setup (NOT directly comparable to the pre-Round-1.2 9-obs MHM of −780.10 due to different observable set). The Round 1.2 b_HtM posterior is **0.114** with 90% HPD **[−0.10, +0.29]** — the HPD crosses zero, so the AU hand-to-mouth consumer channel is at most weak and possibly indistinguishable from zero, well below FR-BDF wp1044's 0.32 calibration. This finding is robust across 4 alternative trend treatments (Option 4 stim-NaN, TVD, FR-BDF, Option α). Detailed comparison in `NEXT_SESSION.md` and working paper §4.11.3. Chains cached in `dynare/au_pac_bayesian/`; alternative-spec chains backed up at `dynare/au_pac_bayesian.cached_*_2026-05-23/`. Cumulative Phase Q → Rounds 4-8 consolidation MHM improvement: **+22.17 nats** (peak under 9-obs baseline; Round 1.2 follow-up runs are on a different observable set).
...
| **Round 1.2 follow-up** (b_HtM estimated, wt_H_real_gap as 10th observable) | 2026-05-23 | 5 alternative trend specifications run; see comparison table below | **b_HtM = 0.08–0.19** across specs (90% HPDs near zero) | Two structural changes: (a) `b_HtM` promoted to `estimated_params` with N(0.30, 0.10) prior, (b) `wt_H_real_gap` added to `varobs` (10th observable). Then 5 MCMC variants run to handle the resulting trend-mismatch issue (post-2008 productivity slowdown vs sample-mean demeaning):  **(1) Unconstrained csminwel** found a boundary-corner mode (γ_w 0.81, α_pc 0.86) — discarded as numerical artifact.  **(2) Seeded from baseline mode** kept Phillips block sensible but acceptance was 3% (jscale issue); b_HtM = 0.096, MHM = −1128.42.  **(3) Option 4: NaN-out 6 stimulus quarters in wt_H_real_gap** (2008Q4-2009Q1, 2020Q1-Q4) to remove the recession-saving-spike bias; b_HtM jumped to 0.19, MHM −1104.79.  **(4) TVD (HP-filter trend) demeaning** of dln_c/dln_ib/pi_w; b_HtM = 0.078, MHM = **−1083.88 (best by 14+ nats)**.  **(5) FR-BDF structural-trend demeaning** (CES Ē with 2 breaks + employment HP-trend); b_HtM = 0.16, MHM = −1098.06.  **(6) Option α: FR-BDF-faithful — trend lives IN the model via `g_bar_C` = 0.498% qoq drift in `dln_c_star_bar` (post-2008 CES BGP); observable matching to `dln_C_obs = dln_c + dln_c_star_bar`; raw data, no demeaning**; b_HtM = 0.114 with 90% HPD [−0.10, +0.29], MHM = −1101.87. Phillips block parameters stabilised somewhere between TVD and FR-BDF. b0_c (ECM speed) collapsed from 0.054 → 0.024 because the trend now lives in the structural target. Wage Phillips still has the trend mismatch (pi_w model SS = 0.625, data mean 1.16% qoq) — to be fixed in Option β with time-varying drift (two breaks at 2002Q2 + 2008Q3 matching the CES Ē regime structure). Both csminwel (mode_compute=4) and newrat (mode_compute=5) crash at `dsge_likelihood` line 763 (dim mismatch in numerical gradient with partial-NaN observations); workaround is `mode_compute=0 + mh_jscale=0.15`. **Robust empirical finding across all 5 estimated-b_HtM specs**: AU's hand-to-mouth channel is in [0.08, 0.19] — much smaller than France's 0.32, and HPDs touch or cross zero in 3 of 5 specs. Cached chains for each spec in `dynare/au_pac_bayesian.cached_{opt4,tvd_hp,frbdf,optalpha}_2026-05-23/`. See NEXT_SESSION.md for pickup instructions. |
